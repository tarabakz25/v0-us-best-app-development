...
export default function PostPage() {
  ...
  const [surveyQuestions, setSurveyQuestions] = useState<Array<{ id: string; question: string; options: string[] }>>([
  ...
  const addSurveyQuestion = () => {
    setSurveyQuestions([...surveyQuestions, { id: Date.now().toString(), question: "", options: ["", ""] }])
  }
  ...
  const updateQuestion = (id: string, question: string) => {
    setSurveyQuestions(surveyQuestions.map((q) => (q.id === id ? { ...q, question } : q)))
  }

  const addOption = (questionId: string) => {
    setSurveyQuestions(surveyQuestions.map((q) => (q.id === questionId ? { ...q, options: [...q.options, ""] } : q)))
  }
  ...
  const handleSubmitAd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (adMedia) {
        ...
        const { data: uploadData, error: uploadError } = await supabase.storage.from("media").upload(fileName, adMedia)
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="max-w-2xl mx-auto p-4">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "ad" | "survey")} className="w-full">
            ...
            <form onSubmit={handleSubmitAd} className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>商品やサービスの魅力を伝える広告を作成しましょう</CardDescription>
                </CardHeader>
            ...
            <form onSubmit={handleSubmitSurvey} className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>ユーザーの意見を集めてより良い商品を作りましょう</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {/* Media Upload */}
                  <div className="space-y-2">
                    ...
                    <div className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary transition-colors cursor-pointer">
                      ...
                      <label htmlFor="survey-media" className="cursor-pointer">
                        {surveyMediaPreview ? (
                          ...
                          <div className="flex flex-col items-center gap-2">
                            <Upload className="w-10 h-10 text-muted-foreground" />
                            <p className="text-sm text-muted-foreground">クリックしてアップロード（任意）</p>
                          </div>
                        )}
                      </label>
                    </div>
                  ...
                  <Card key={question.id}>
                    <CardHeader className="pb-3">
                      <div className="flex items-start justify-between">
                        <CardTitle className="text-base">質問 {qIndex + 1}</CardTitle>
                    ...
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                        <Label htmlFor={`question-${question.id}`}>質問文*</Label>
                        <Input
